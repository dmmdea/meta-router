...
func TestLoadValidatesAndMarksProvisioning(t *testing.T) {
	dir := t.TempDir()
	home := filepath.Join(dir, "claude-acct2")
	if err := os.MkdirAll(home, 0o755); err != nil {
		t.Fatal(err)
	}
	// acct2 home exists but has NO credentials → listed, typed not_provisioned.
	regPath := write(t, dir, "profiles.json", `{
		"claude": [
			{"subject": "default", "home": ""},
			{"subject": "acct2", "home": "`+filepath.ToSlash(home)+`"}
		]
	}`)
	...
	if ps[0].Subject != "default" || !ps[0].Provisioned {
		t.Fatalf("default must be provisioned (live home), got %+v", ps[0])
	}
	if ps[1].Subject != "acct2" || ps[1].Provisioned {
		t.Fatalf("credential-less home must be marked NOT provisioned, got %+v", ps[1])
	}
	// Provision it → flips.
	write(t, home, ".credentials.json", `{"claudeAiOauth":{"accessToken":"x"}}`)
	reg2, _ := Load(regPath)
	if !reg2.Lane("claude")[1].Provisioned {
		t.Fatal("credentialed home must be provisioned")
	}
	// codex was absent from the file → implicit default preserved.
