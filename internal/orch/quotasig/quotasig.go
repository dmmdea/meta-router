...
// observation. The tee rewrites the file on every render (atomic tmp+rename, so
// mtime is the content-write time and a partial file is never visible), which
// makes mtime a sound freshness bound. A drop older than this is a dead file or
// a seeded fixture, never current pressure.
//
// DEFERRED (audit 2026-07-25, m4): an ANCHOR sanity bound — a 5h window whose
// resets_at lies beyond now+5h is impossible, and that was the seeded fixture's
// other tell. Not added here: three existing fixtures anchor a 5h window three
// days out, so the check needs those fixtures made realistic first, and a
// fixture edit inside this PR could hide a regression. Own change, behavior
// pinned.
...
func IngestTraced(l *ledger.Ledger, path, tracePath, lane string, now time.Time) (int, string, error) {
	...
	if serr != nil {
		// No mtime ⇒ no freshness evidence. The drop is optional signal, so
		// skipping breaks no invariant, while defaulting to "fresh" would
		// bypass the guard entirely.
		return 0, "statusline drop mtime unavailable: skipped (" + serr.Error() + ")", nil
	}
	obsAt := fi.ModTime().UTC()
	if obsAt.After(now) {
		obsAt = now // clamp a future mtime: it must not confer permanent freshness
	}
	...
	for _, o := range obs {
		...
		applied, why := l.Observe(ledger.Observation{Lane: lane, Window: o.Window, UsedPct: o.UsedPct,
			ResetsAt: o.ResetsAt, ObservedAt: obsAt, Source: ledger.ProviderSourceDrop}, now)
		if applied {
			n++
		} else if why != "" {
			note = "statusline drop " + string(o.Window) + ": " + why
		}
		// The trace row is a MEASUREMENT, not an authority: it is appended even
		// when the ledger write was refused, because quota-parity exists to
		// compare the two surfaces. Suppressing refused rows made the drop side
		// vanish from the parity report in steady state.
...
func ApplySnapshotsSubject(l *ledger.Ledger, subject string, snaps []quotapoll.Snapshot, tracePath, origin string, now time.Time) (int, string) {
	...
	for _, s := range snaps {
		...
		applied, why := l.Observe(ledger.Observation{Lane: s.Lane, Subject: subject, Window: s.Window,
			UsedPct: s.UsedPct, ResetsAt: s.ResetsAt, ObservedAt: now,
			Source: ledger.ProviderSourcePoll}, now)
		if applied {
			n++
		} else if why != "" {
			// A refused POLL is a real anomaly (clock skew, a vendor denial in
			// force) and must never be silent.
			note = s.Lane + "/" + string(s.Window) + " poll refused: " + why
		}
		if tracePath != "" && changed {
