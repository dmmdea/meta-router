...
func TestScore(t *testing.T) {
	...
	if m.RecallAt[1] != 0.0 {
		t.Fatalf("recall@1 should be 0 (right was ranked 2nd), got %v", m.RecallAt[1])
	}
	if m.RecallAt[3] != 0.5 {
		t.Fatalf("recall@3 should be 0.5 (1 of 2 hit), got %v", m.RecallAt[3])
	}
	if m.MRR <= 0 {
		t.Fatalf("MRR should reflect the rank-2 hit, got %v", m.MRR)
	}
}
