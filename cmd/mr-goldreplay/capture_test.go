...
func TestTruncateDiffReceiptsCutLines(t *testing.T) {
	...
	if _, cut := truncateDiffN(clean + "\nThat should fix it.\nLet me know.\n"); cut != 4 {
		t.Fatalf("prose tail must be counted (blank + 2 lines + trailing), got %d", cut)
	}
}
...
func TestReplayOneRecordsDiffProvenance(t *testing.T) {
	...
	if row.OutcomeClass != "ok" || !row.VerifierPass || row.DiffSource != diffSourceWorktree {
	...
	// diff plus trailing prose (2 lines cut). The same apply failure stays
	// a measured failure.
	...
	row = replayOne(task, cfg, 1, os.Args[0], os.Args[0], repos, 30, 10, "")
	if row.OutcomeClass != "ok" || row.VerifierPass || row.DiffSource != diffSourcePrinted || row.DiffTruncatedLines != 3 {
