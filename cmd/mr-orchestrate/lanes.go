...
func applyCodexOutcome(l *ledger.Ledger, o codexlane.Outcome, cfg orchcfg.Config, now time.Time, gate codex5hGate) (predictedUsedPct float64) {
	predictedUsedPct = -1
	if b, ok := l.Bucket("codex", ledger.Win5h); ok {
		predictedUsedPct = b.UsedPct
	}
	if gate.Suppress {
		// The 5h estimate is a phantom on this plan (corroborated absence):
...
func runCodexLane(out io.Writer, prompt, model, effort, cwd string, timeoutSec int, extra []string, live, force, keepHome bool, origin, desc string, rf recFields, sf strategyFields) (int, error) {
	...
	predicted := -1.0
	gate := codex5hEstimateGate(cfg, loadPollState(), now)
	warnIf(updateLedger(func(fresh *ledger.Ledger) {
		predicted = applyCodexOutcome(fresh, o, cfg, now, gate)
