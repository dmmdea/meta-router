	...
	return []map[string]any{
		{
			...
			"inputSchema": map[string]any{
				...
				"properties": map[string]any{
					...
					"exclude":           arrProp("lanes to mask for THIS consult only (claude|codex|copilot|glm|local|groq|cloudflare|openrouter|nim|gemini, or the group alias free); delegate-mode passes [\"claude\"]"),
				},
		...
		},
		{
			...
			"inputSchema": map[string]any{
				...
				"properties": map[string]any{
					...
					"lane":        strProp("dispatch lane: auto|claude|codex|copilot|glm|groq|cloudflare|openrouter|nim|gemini (default auto; glm is retired unless re-enabled in config; free-provider lanes need a credential file under <state>/free/)"),
					...
					"batch":       boolProp("E2 spend-down tag: an already-queued BATCH task (never set for interactive work); enables the under-utilized-window rank boost"),
					"est_minutes": numProp("expected task duration in minutes (E2 completion-fit gate; 0 = unknown → no boost)"),
					"exclude":     arrProp("lanes to mask for this dispatch and refuse if resolved (claude|codex|copilot|glm|local|groq|cloudflare|openrouter|nim|gemini, or the group alias free); delegate-mode passes [\"claude\"]"),
				},
